function dial_widgetlist()
{
  var widgets = {
    "dial":
    {
      "offsetx":-80,"offsety":-80,"width":250,"height":250,
      "menu":"Widgets",
...
function dial_fastupdate()
{
  ...
  pos -= width;
  ctx.lineWidth = (size*0.025).toFixed(0);
  pos += width;
  ctx.strokeStyle = "#fff";
  ctx.beginPath();
  ...
  ctx.lineWidth = (size*0.010).toFixed(0);
  //---------------------------------------------------------------
  ctx.beginPath();
  ctx.moveTo(x+Math.sin(Math.PI*a-0.20)*inner,y+Math.cos(Math.PI*a-0.20)*inner); /*edited by praveen*/
  ctx.lineTo(x+Math.sin(Math.PI*a-0.10)*size,y+Math.cos(Math.PI*a-0.10)*size); 
  ctx.lineTo(x+Math.sin(Math.PI*a)*inner,y+Math.cos(Math.PI*a)*inner); 
  ctx.arc(x,y,inner,1-(Math.PI*a-0.3),1-(Math.PI*a+5.4),true);
  ctx.closePath();
  ctx.fill();
  ctx.stroke();
  ...
  ctx.fillText(position+units,x,y-10+(size*0.125));

...
function auto_configure_inputs($userid, $id, $name)
{
  ...
  if (preg_match("/power/i", $name) || preg_match("/solar/i", $name) || preg_match("/wind/i", $name))
  {
    $feedid = create_feed($userid, $name, DataType::REALTIME);
    add_input_process($userid, $id, 1, $feedid);
	
	$feedid = create_feed($userid, $name."-kwh", DataType::REALTIME);
    add_input_process($userid, $id, 4, $feedid);
